...
class EnsureAntivirusProfileConfiguredRule(BaseRule):
    ...
    def evaluate(self, parsed_config: Dict[str, Any]) -> RuleResult:
        av_profiles = parsed_config.get("config antivirus profile") or parsed_config.get("antivirus profile", {})
        ...
        if isinstance(av_profiles, dict):
            for profile_name, profile_data in av_profiles.items():
                if isinstance(profile_data, dict):
                    # Verificar si la inspección HTTP está activa
                    http_config = profile_data.get("http", {})
                    http_options = http_config.get("options", "") if isinstance(http_config, dict) else ""
                    
                    if "scan" not in http_options:
...
class EnsureWebFilterCategoriesRule(BaseRule):
    ...
    def evaluate(self, parsed_config: Dict[str, Any]) -> RuleResult:
        wf_profiles = parsed_config.get("config webfilter profile") or parsed_config.get("webfilter profile", {})
...
class EnsureIPSSensorConfiguredRule(BaseRule):
    ...
    def evaluate(self, parsed_config: Dict[str, Any]) -> RuleResult:
        ips_sensors = parsed_config.get("config ips sensor") or parsed_config.get("ips sensor", {})
        ...
        return RuleResult(
            ...
            expected_value="Sensors IPS bloqueando firmas de alta severidad",
        )
...
class EnsureAppControlConfiguredRule(BaseRule):
    ...
    def evaluate(self, parsed_config: Dict[str, Any]) -> RuleResult:
        app_list = parsed_config.get("config application list") or parsed_config.get("application list", {})
...
class EnsureDNSFilterConfiguredRule(BaseRule):
    ...
    def evaluate(self, parsed_config: Dict[str, Any]) -> RuleResult:
        dns_profiles = parsed_config.get("config dnsfilter profile") or parsed_config.get("dnsfilter profile", {})
...
class EnsureBotnetIPBlockingRule(BaseRule):
    ...
    def evaluate(self, parsed_config: Dict[str, Any]) -> RuleResult:
        # Revisa en las interfaces la propiedad scan-botnet-connections
            ...
            for iface_name, iface_data in interface_config.items():
                if isinstance(iface_data, dict):
                    ...
                    if role == "wan" or iface_name.lower().startswith("wan"):
                        botnet_scan = iface_data.get("scan-botnet-connections", "block")
...
class EnsureFortiSandboxIntegrationRule(BaseRule):
    ...
    def evaluate(self, parsed_config: Dict[str, Any]) -> RuleResult:
        sandbox_config = parsed_config.get("config system fortisandbox") or parsed_config.get("system fortisandbox", {})

        status = sandbox_config.get("status", "disable")
