# app/integrations/fortigate/fortios_v7_2.py

"""Adaptadores concretos de comunicación para FortiOS v7.2.x."""
...
class FortiOSV72DeviceConnector(DeviceConnector):
    """Conector optimizado para FortiOS v7.2.x (System Scope)."""
    ...
    async def test_connectivity(self) -> FortiGateConnectivityResult:
        try:
            ...
            return FortiGateConnectivityResult(
                ...
                version=None,
                serial=None,
                message=f"Error de API ({e.status_code}): Token o permisos inválidos",
            )
            ...
            return FortiGateConnectivityResult(
                ...
                version=None,
                serial=None,
                message="Timeout/Error de conexión contra el puerto del dispositivo",
            )
...
class FortiOSV72VDOMConnector(VDOMConnector):
    """Conector VDOM optimizado para FortiOS v7.2.x (Tenant Scope)."""

    def __init__(
        self, host: str, port: int, token: str, vdom: str, verify_ssl: bool = False
    ):
    ...
    async def test_connectivity(self) -> FortiGateConnectivityResult:
        try:
            ...
            return FortiGateConnectivityResult(
                ...
                version=None,
                serial=None,
                message=f"VDOM {self.vdom} OK (v7.2)",
            )
        except Exception as e:
            return FortiGateConnectivityResult(
                is_reachable=False,
    ...
    async def get_firewall_policies(self) -> List[Dict[str, Any]]:
        data = await self.http.get(
            "/api/v2/cmdb/firewall/policy", params={"vdom": self.vdom}
        )
        return data.get("results", [])

    async def get_interfaces(self) -> List[Dict[str, Any]]:
        data = await self.http.get(
            "/api/v2/cmdb/system/interface", params={"vdom": self.vdom}
        )
        return data.get("results", [])

    async def get_admin_settings(self) -> Dict[str, Any]:
        return await self.http.get(
            "/api/v2/cmdb/system/admin", params={"vdom": self.vdom}
        )

    async def get_ipsec_tunnels(self) -> List[Dict[str, Any]]:
        data = await self.http.get(
            "/api/v2/cmdb/vpn.ipsec/phase1-interface", params={"vdom": self.vdom}
        )
        return data.get("results", [])
