# app/integrations/fortigate/fortios_v7_0.py

"""Adaptadores concretos de comunicación para FortiOS v7.0.x."""
...
class FortiOSV70DeviceConnector(DeviceConnector):
    """Conector optimizado para FortiOS v7.0.x (System Scope)."""
    ...
    async def test_connectivity(self) -> FortiGateConnectivityResult:
        try:
            ...
            return FortiGateConnectivityResult(
                is_reachable=True,
                status_code=200,
                version=results.get("version", "v7.0.x"),
                serial=results.get("serial", "Unknown"),
                message="Conexión exitosa contra FortiOS 7.0",
            )
...
class FortiOSV70VDOMConnector(VDOMConnector):
    """Conector VDOM optimizado para FortiOS v7.0.x (Tenant Scope)."""
    ...
    async def test_connectivity(self) -> FortiGateConnectivityResult:
        try:
            ...
            return FortiGateConnectivityResult(
                ...
                version=None,
                serial=None,
                message=f"VDOM {self.vdom} OK (v7.0)",
            )
        except Exception as e:
            return FortiGateConnectivityResult(
                is_reachable=False,
